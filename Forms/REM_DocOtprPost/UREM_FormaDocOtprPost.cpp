...
//---------------------------------------------------------------
int TREM_FormaDocOtprPost::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
if (number_procedure_end==ER_Nom)
		...
		if (type_event==1)
				...
				if (DM->DocTKOL_REM_DOTPRPOSTT->AsFloat > 0.001)  //задано количество
					{ //не изменяем
					}
				else
					{
...
if (fl_create_doc == true)
		...
		doc->DM->DocPRIM_REM_DPRN->AsString="На основании отправки поставщиком(ремонт) №"+DM->DocAllNUM_REM_GALLDOC->AsString+

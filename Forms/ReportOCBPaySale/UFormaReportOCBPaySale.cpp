...
//---------------------------------------------------------------------------
int TFormaReportOCBPaySale::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
cell->Text = "Отчет по кредитам "+DateToStr(DateNach->Date)
...
if (IdDogovor>0)
        {
        cell = prForm->SS->ActiveSheet->GetCellObject(2, numRow);
        cell->Text = "договор: "+NameDogovorEdit->Text;
        cell->Style->Font->Size = 12;
        delete cell;
        numRow++;
        }
...
cell->Text = "Фирма / Контрагент / Договор / Документ реал.";
cell->Style->Font->Size = 12;
cell->Style->HorzTextAlign = haCENTER;
cell->Style->VertTextAlign = vaCENTER;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
...
//номенлатура
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text = glGetNameDoc(IBQTDOC_PROC->AsString)+" №"+IBQNUMDOC_PROC->AsString+" от "+IBQPOSDOC_PROC->AsString;
        cell->Style->Font->Size = 10;
cell->Style->HorzTextAlign = haLEFT;
cell->Style->VertTextAlign = vaCENTER;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
...
if (FormaSpiskaSprDogovor==NULL)
		...
		//FormaSpiskaSprDogovor->LabelKlient->Caption="Контрагент: "+NameKlientEdit->Text;

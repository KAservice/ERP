object FormaReportOCBPaySale: TFormaReportOCBPaySale
  Left = 433
  Top = 274
  Caption = #1056#1077#1072#1083#1080#1079#1072#1094#1080#1103' '#1087#1086' '#1086#1087#1083#1072#1090#1077':'
  ClientHeight = 458
  ClientWidth = 577
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -14
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 120
  TextHeight = 16
  object Label1: TLabel
    Left = 30
    Top = 10
    Width = 298
    Height = 32
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1056#1077#1072#1083#1080#1079#1072#1094#1080#1103' '#1087#1086' '#1086#1087#1083#1072#1090#1077
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -28
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentColor = False
    ParentFont = False
  end
  object Label2: TLabel
    Left = 20
    Top = 59
    Width = 14
    Height = 32
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = 'c'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -28
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label3: TLabel
    Left = 276
    Top = 59
    Width = 32
    Height = 32
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1087#1086
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -28
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label6: TLabel
    Left = 39
    Top = 142
    Width = 47
    Height = 16
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1060#1080#1088#1084#1072':'
  end
  object Label4: TLabel
    Left = 39
    Top = 187
    Width = 50
    Height = 16
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1043#1088#1091#1087#1087#1072':'
  end
  object Label14: TLabel
    Left = 10
    Top = 226
    Width = 93
    Height = 20
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1050#1086#1085#1090#1088#1072#1075#1077#1085#1090':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -17
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label5: TLabel
    Left = 49
    Top = 266
    Width = 55
    Height = 16
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1044#1086#1075#1086#1074#1086#1088
  end
  object Label10: TLabel
    Left = 315
    Top = 307
    Width = 194
    Height = 16
    Caption = '* '#1090#1086#1083#1100#1082#1086' '#1076#1083#1103' '#1074#1085#1077#1096#1085#1080#1093' '#1086#1090#1095#1077#1090#1086#1074
  end
  object cxButtonCreate: TcxButton
    Left = 286
    Top = 418
    Width = 132
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1057#1092#1086#1088#1084#1080#1088#1086#1074#1072#1090#1100
    TabOrder = 8
    OnClick = cxButtonCreateClick
  end
  object cxButtonClose: TcxButton
    Left = 425
    Top = 418
    Width = 93
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Action = ActionClose
    TabOrder = 9
  end
  object NameDogovorEdit: TcxButtonEdit
    Left = 119
    Top = 267
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameDogovorEditPropertiesButtonClick
    TabOrder = 5
    Text = 'NameDogovorEdit'
    Width = 391
  end
  object NameKlientEdit: TcxButtonEdit
    Left = 119
    Top = 225
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameKlientEditPropertiesButtonClick
    TabOrder = 4
    Text = 'NameKlientEdit'
    Width = 391
  end
  object NameGrpKlientEdit: TcxButtonEdit
    Left = 121
    Top = 183
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameGrpKlientEditPropertiesButtonClick
    TabOrder = 3
    Text = 'NameGrpKlientEdit'
    Width = 389
  end
  object NameFirmEdit: TcxButtonEdit
    Left = 119
    Top = 138
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameFirmEditPropertiesButtonClick
    TabOrder = 2
    Text = 'NameFirmEdit'
    Width = 391
  end
  object DateNach: TcxDateEdit
    Left = 42
    Top = 70
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Kind = ckDateTime
    TabOrder = 0
    Width = 215
  end
  object DateCon: TcxDateEdit
    Left = 335
    Top = 70
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Kind = ckDateTime
    TabOrder = 1
    Width = 208
  end
  object cxButton1: TcxButton
    Left = 147
    Top = 418
    Width = 132
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1042#1085#1077#1096#1085#1080#1081' '#1086#1090#1095#1077#1090
    DropDownMenu = PopupMenuExtModule
    Glyph.Data = {
      
      0E9322280A931E1E01920505001B000000000000000000000000}
    Kind = cxbkDropDown
    TabOrder = 10
  end
  object cxLabel9: TcxLabel
    Left = 1
    Top = 331
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1041#1080#1079#1085#1077#1089' '#1086#1087#1077#1088#1072#1094#1080#1103'*:'
  end
  object cxLabel3: TcxLabel
    Left = 35
    Top = 363
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1055#1088#1086#1077#1082#1090'*:'
  end
  object NameBusinessOpercxButtonEdit: TcxButtonEdit
    Left = 135
    Top = 330
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ClickKey = 13
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameBusinessOpercxButtonEditPropertiesButtonClick
    TabOrder = 6
    Text = 'NameBusinessOpercxButtonEdi'
    Width = 375
  end
  object NameProjectcxButtonEdit: TcxButtonEdit
    Left = 135
    Top = 362
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ClickKey = 13
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameProjectcxButtonEditPropertiesButtonClick
    TabOrder = 7
    Text = 'NameProjectcxButtonEdit'
    Width = 375
  end
  object IBQ: TpFIBDataSet
    SelectSQL.Strings = (
      '         select   *'
      ''
      
        'from OCB_RGPAYSALE_FULL_REPORT (:PARAM_POSNACH, :PARAM_POSCON, :' +
        'PARAM_IDFIRM, :PARAM_IDGRPKLIENT, :PARAM_IDKLIENT, :PARAM_IDDOGO' +
        'VOR)'
      ''
      '')
    Transaction = IBTr
    Left = 160
    Top = 264
    object IBQNUMDOC_PROC: TFIBIntegerField
      FieldName = 'NUMDOC_PROC'
    end
    object IBQPOSDOC_PROC: TFIBDateTimeField
      FieldName = 'POSDOC_PROC'
    end
    object IBQDATE_PROC: TFIBDateField
      FieldName = 'DATE_PROC'
    end
    object IBQNACHOST_PROC: TFIBBCDField
      FieldName = 'NACHOST_PROC'
      Size = 2
      RoundByScale = True
    end
    object IBQPRIHOD_PROC: TFIBBCDField
      FieldName = 'PRIHOD_PROC'
      Size = 2
      RoundByScale = True
    end
    object IBQRASHOD_PROC: TFIBBCDField
      FieldName = 'RASHOD_PROC'
      Size = 2
      RoundByScale = True
    end
    object IBQCONOST_PROC: TFIBBCDField
      FieldName = 'CONOST_PROC'
      Size = 2
      RoundByScale = True
    end
    object IBQIDFIRM_PROC: TFIBBCDField
      FieldName = 'IDFIRM_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQIDGRP_PROC: TFIBBCDField
      FieldName = 'IDGRP_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQIDKLIENT_PROC: TFIBBCDField
      FieldName = 'IDKLIENT_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQIDDOGOVOR_PROC: TFIBBCDField
      FieldName = 'IDDOGOVOR_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQIDDOC_PROC: TFIBBCDField
      FieldName = 'IDDOC_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQNAMEFIRM_PROC: TFIBWideStringField
      FieldName = 'NAMEFIRM_PROC'
      Size = 100
    end
    object IBQNAMEGRP_PROC: TFIBWideStringField
      FieldName = 'NAMEGRP_PROC'
      Size = 100
    end
    object IBQNAMEKLIENT_PROC: TFIBWideStringField
      FieldName = 'NAMEKLIENT_PROC'
      Size = 100
    end
    object IBQNAMEDOGOVOR_PROC: TFIBWideStringField
      FieldName = 'NAMEDOGOVOR_PROC'
      Size = 100
    end
    object IBQTDOC_PROC: TFIBWideStringField
      FieldName = 'TDOC_PROC'
      Size = 10
    end
  end
  object IBTr: TpFIBTransaction
    TimeoutAction = TARollback
    TRParams.Strings = (
      'read'
      'nowait'
      'read_committed'
      'rec_version')
    TPBMode = tpbDefault
    Left = 128
    Top = 264
  end
  object ActionList: TActionList
    Images = DM.ImageList1
    Left = 40
    Top = 16
    object ActionOpenHelp: TAction
      Caption = 'ActionOpenHelp'
      ShortCut = 112
      OnExecute = ActionOpenHelpExecute
    end
    object ActionClose: TAction
      Caption = #1047#1072#1082#1088#1099#1090#1100
      ImageIndex = 53
      OnExecute = ActionCloseExecute
    end
  end
  object PopupMenuExtModule: TPopupMenu
    Left = 40
    Top = 272
  end
end

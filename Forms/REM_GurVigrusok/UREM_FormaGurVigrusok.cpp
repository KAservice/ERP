...
//---------------------------------------------------------------------------
__fastcall TREM_FormaGurVigrusok::TREM_FormaGurVigrusok(TComponent* Owner)
		: TForm(Owner)
{
...
//---------------------------------------------------------------------------
bool TREM_FormaGurVigrusok::Init(void)
{
bool result=false;
...
InterfaceGlobalCom->kanCreateObject(ProgId_REM_DMGurVigrusok,IID_IREM_DMGurVigrusok, (void**)&DM);
...
DMTableExtPrintForm->OpenTable(StringToGUID(Global_CLSID_TREM_FormaGurVigrusokImpl),false);
...
//---------------------------------------------------------------------------
int TREM_FormaGurVigrusok::Done(void)
{

return -1;
}
//---------------------------------------------------------------------------
void TREM_FormaGurVigrusok::UpdateForm(void)
{
...
//---------------------------------------------------------------------------
void TREM_FormaGurVigrusok::OpenTable(void)
{

}
//----------------------------------------------------------------------------

void __fastcall TREM_FormaGurVigrusok::FormClose(TObject *Sender,
	  TCloseAction &Action)
{
...
//---------------------------------------------------------------------------
int TREM_FormaGurVigrusok::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
//--------------------------------------------------------------------------
void TREM_FormaGurVigrusok::OpenFormDoc()
{
		...
		//настройка формы
		doc->DM->OpenDoc(glStrToInt64(DM->TableID_REM_GUR_VIGR->AsString));
		doc->UpdateForm();
...
void __fastcall TREM_FormaGurVigrusok::ToolButtonDeleteClick(TObject *Sender)
{
DeleteDoc();
}
//---------------------------------------------------------------------------
void TREM_FormaGurVigrusok::DeleteDoc(void)
{

		IREM_DMDocVigruska * doc;
		InterfaceGlobalCom->kanCreateObject(ProgId_REM_DMDocVigruska,IID_IREM_DMDocVigruska,
													(void**)&doc);
		doc->Init(InterfaceMainObject,InterfaceImpl);
		if (!doc) return;
		//настройка формы
		doc->OpenDoc(glStrToInt64(DM->TableID_REM_GUR_VIGR->AsString));

		String V="Вы действительно хотите удалить документ Выгрузка №"
		+doc->DocAllNUM_REM_GUR_VIGR->AsString+" от "+doc->DocAllPOS_REM_GUR_VIGR->AsString+"?";
		...
		if (Application->MessageBox(V.c_str(),Z.c_str(),MB_YESNO)!=IDYES)
				...
				if (doc->DeleteDoc(glStrToInt64(DM->TableID_REM_GUR_VIGR->AsString))==true)
						{
						DM->UpdateTable();
						}
				else
...
//----------------------------------------------------------------------------
void __fastcall TREM_FormaGurVigrusok::ToolButton3Click(TObject *Sender)
{
...
//-----------------------------------------------------------------------------
void __fastcall TREM_FormaGurVigrusok::ToolButtonViborPeriodaClick(
	  TObject *Sender)
{
...
void __fastcall TREM_FormaGurVigrusok::ToolButtonOpenDocClick(TObject *Sender)
{
OpenFormDoc();        
}
//---------------------------------------------------------------------------
...
void __fastcall TREM_FormaGurVigrusok::ToolButtonOtborClick(TObject *Sender)
{
...
void __fastcall TREM_FormaGurVigrusok::ToolButtonParamOtborClick(TObject *Sender)
{
...
void __fastcall TREM_FormaGurVigrusok::ToolButtonRefreshClick(TObject *Sender)
{
DM->UpdateTable();
}
//---------------------------------------------------------------------------
...
void __fastcall TREM_FormaGurVigrusok::cxGrid1DBTableView1DblClick(TObject *Sender)
{
if (Vibor==true)
	{
	TypeEvent=1;
...
void __fastcall TREM_FormaGurVigrusok::cxGrid1DBTableView1KeyPress(TObject *Sender,
      char &Key)
{
if (Key==VK_RETURN)
		{
...
//---------------------------------------------------------------------------
//Внешние модули
void __fastcall TREM_FormaGurVigrusok::PopupMenuExtModuleClick(TObject *Sender)
{
...
//----------------------------------------------------------------------------
void TREM_FormaGurVigrusok::RunExternalModule(__int64 id_module, int type_module)
{
...
module->SetInt64Variables("glIdDoc", glStrToInt64(DM->TableID_REM_GUR_VIGR->AsString));
...
void __fastcall TREM_FormaGurVigrusok::ToolButton6Click(TObject *Sender)
{
OpenFormNewDoc();
}
//---------------------------------------------------------------------------
void TREM_FormaGurVigrusok::OpenFormNewDoc()
{

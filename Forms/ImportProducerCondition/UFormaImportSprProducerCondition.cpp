...
//---------------------------------------------------------------------------
int TFormaImportSprProducerCondition::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
//			}
		...
		dm_producer_condition->Element->Post();
				...
				if (dm_producer_condition->SaveElement()==false)
					{
					text_error="Ошибка при записи новой номенклатуры: "+dm_producer_condition->TextError+"\n";
...
//			}
		...
		if(id_producer_condition==0) //создаем новую номенклатуру
			{
			//*********************************************************************************
			dc_upd->AppendRecord();

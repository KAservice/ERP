...
//---------------------------------------------------------------------------
int TREM_FormaDocZOper::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
			 ...
			 //	DM->DocTTNOM->AsInteger=FormaSpiskaSprNom->DM->ElementTNOM->AsInteger;
				...
				DM->DocTKF_REM_DZOPERT->AsFloat=FormaSpiskaSprNom->DM->OsnEdKFED->AsFloat;
					...
					DM->DocTPRICE_REM_DZOPERT->AsFloat=
							DMSprPrice->GetValuePrice(glStrToInt64(DM->DocIDTPRICE_REM_DZOPER->AsString), //тип цен
											glStrToInt64(DM->DocTIDNOM_REM_DZOPERT->AsString),            //номенклатура
											glStrToInt64(DM->DocTIDED_REM_DZOPERT->AsString),             //единица
											DM->DocTKF_REM_DZOPERT->AsFloat,                              //коэффициент

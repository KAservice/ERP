...
//---------------------------------------------------------------------------
__fastcall TFormaDocRegOstNom::TFormaDocRegOstNom(TComponent* Owner)
		: TForm(Owner)
{
...
//---------------------------------------------------------------------------
bool TFormaDocRegOstNom::Init(void)
{
bool result=false;
...
InterfaceGlobalCom->kanCreateObject("Oberon.DMDocRegOstNom.1",IID_IDMDocRegOstNom, (void**)&DM);
...
DMTableExtPrintForm->OpenTable(StringToGUID(Global_CLSID_TFormaDocRegOstNomImpl),false);
...
//---------------------------------------------------------------------------
int TFormaDocRegOstNom::Done(void)
{

return -1;
}
//----------------------------------------------------------------------------
void TFormaDocRegOstNom::UpdateForm(void)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::FormClose(TObject *Sender,
      TCloseAction &Action)
{
...
//---------------------------------------------------------------------------
int TFormaDocRegOstNom::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
void TFormaDocRegOstNom::ViborInfBase(void)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborInfBase(void)
{
...
//---------------------------------------------------------------------------
//выбор фирмы
void TFormaDocRegOstNom::ViborFirm(void)
{
if(Prosmotr==true) return;
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborFirm(void)
{
	...
	DM->DocT->Edit();
	DM->DocTIDFIRM_DRG_OSTNOMT->AsString=FormaSpiskaSprFirm->DM->ElementIDFIRM->AsString;
...
//-----------------------------------------------------------------------------
//выбор склада
void TFormaDocRegOstNom::ViborSklad(void)
{
if(Prosmotr==true) return;
...
//-----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborSklad(void)
{


	DM->DocT->Edit();
	DM->DocTIDSKLAD_DRG_OSTNOMT->AsString=FormaSpiskaSprSklad->DM->ElementIDSKLAD->AsString;
...
//----------------------------------------------------------------------------
//выбор номенклатуры
void TFormaDocRegOstNom::ViborNom(void)
{
if(Prosmotr==true) return;
...
//		+DM->DocAllNUMDOC->AsString+" от "+DM->DocAllPOSDOC->AsString;
		FormaSpiskaSprNom->IdNom=glStrToInt64(DM->DocTIDNOM_DRG_OSTNOMT->AsString);
		FormaSpiskaSprNom->UpdateForm();
		}
}
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborNom(void)
{

				DM->DocT->Edit();
				DM->DocTIDNOM_DRG_OSTNOMT->AsString=FormaSpiskaSprNom->DM->ElementIDNOM->AsString;
				DM->DocTNAMENOM->AsString=FormaSpiskaSprNom->DM->ElementNAMENOM->AsString;
				DM->DocTKOL_DRG_OSTNOMT->AsInteger=1;
...
//--------------------------------------------------------------------------
//выбор партия
void TFormaDocRegOstNom::ViborPart(void)
{ 
if(Prosmotr==true) return;
if (FormaSpiskaSprPart==NULL)
		{
		...
		FormaSpiskaSprPart->NumberProcVibor=ER_Part;
		FormaSpiskaSprPart->Vibor=true;
		FormaSpiskaSprPart->IdNom=glStrToInt64(DM->DocTIDNOM_DRG_OSTNOMT->AsString);
		FormaSpiskaSprPart->DM->OpenTable(glStrToInt64(DM->DocTIDNOM_DRG_OSTNOMT->AsString));
		//FormaSpiskaSprPart->LabelNom->Caption="Партии номенклатуры: "+DM->DocTNAMENOM->AsString;
		}
}
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborPart(void)
{

				DM->DocT->Edit();
				DM->DocTIDPART_DRG_OSTNOMT->AsString=FormaSpiskaSprPart->DM->ElementIDPART->AsString;
				DM->DocTDOCPART->AsString=FormaSpiskaSprPart->DM->ElementDOCPART->AsString;
				DM->DocTKOL_DRG_OSTNOMT->AsInteger=1;
				DM->DocT->Post();
...
//----------------------------------------------------------------------------
void TFormaDocRegOstNom::OpenPrintForm(void)
{
...
//---------------------------------------------------------------------------
void TFormaDocRegOstNom::OutputZagolovokReport(TSheetEditor *prForm)
{
...
cell->Text="Движения регистра Остатки партий товаров №"+DM->DocAllNUMDOC->AsString+" от "
...
cell->Text="Основание: "+DM->DocDESCR_DRG_OSTNOM->AsString;
delete cell;
numRow++;
numRow++;
...
//------------------------------------------------------------------------------
void TFormaDocRegOstNom::OutputString(TSheetEditor *prForm)
{
numRow ++;
...
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text=DM->DocTDOCPART->AsString;
cell->Style->WordBreak = true;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
cell->Style->Borders->Edges [eRight]->Style=lsThin;
cell->Style->Borders->Edges [eTop]->Style=lsThin;
...
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text=DM->DocTKOL_DRG_OSTNOMT->AsString;
...
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text=DM->DocTSUM_DRG_OSTNOMT->AsString;
cell->Style->HorzTextAlign = haRIGHT;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
cell->Style->Borders->Edges [eRight]->Style=lsThin;
cell->Style->Borders->Edges [eTop]->Style=lsThin;
...
//------------------------------------------------------------------------------
void TFormaDocRegOstNom::OutputPodavalReport(TSheetEditor *prForm)
{
numRow++;
numCol=2;
TcxSSCellObject *cell;
...
void __fastcall TFormaDocRegOstNom::NumberDocKeyDown(TObject *Sender,
...
void __fastcall TFormaDocRegOstNom::FDateDocKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN) FindNextControl((TWinControl *) Sender,true,true,false)->SetFocus();
}
...
void __fastcall TFormaDocRegOstNom::PrimKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
if(Key==VK_RETURN) FindNextControl((TWinControl *) Sender,true,true,false)->SetFocus();
}
...
void __fastcall TFormaDocRegOstNom::ActionOpenHelpExecute(TObject *Sender)
{
Application->HelpJump("DocRegOstNom");
}
//---------------------------------------------------------------------------



void __fastcall TFormaDocRegOstNom::NameInfBasecxDBButtonEditPropertiesButtonClick(
...
void __fastcall TFormaDocRegOstNom::ActionAddStringExecute(TObject *Sender)
{
if(Prosmotr==true) return;
...
void __fastcall TFormaDocRegOstNom::ActionDeleteStringExecute(TObject *Sender)
{
if(Prosmotr==true) return;
DM->TableDelete();
}
...
void __fastcall TFormaDocRegOstNom::NumDoccxDBTextEditKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN)
	{
...
void __fastcall TFormaDocRegOstNom::PosDoccxDBDateEditKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN)
	{
...
void __fastcall TFormaDocRegOstNom::PrimcxDBTextEditKeyDown(TObject *Sender,
	  WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN)
	{
...
void TFormaDocRegOstNom::BeforeSaveDoc(void)
{
...
void __fastcall TFormaDocRegOstNom::cxGrid1DBTableView1NAMEFIRMPropertiesButtonClick(
      TObject *Sender, int AButtonIndex)
{
ViborFirm();	
}
...
void __fastcall TFormaDocRegOstNom::cxGrid1DBTableView1NAMESKLADPropertiesButtonClick(
      TObject *Sender, int AButtonIndex)
{
ViborSklad();	
}
...
void __fastcall TFormaDocRegOstNom::cxGrid1DBTableView1NAMENOMPropertiesButtonClick(
      TObject *Sender, int AButtonIndex)
{
ViborNom();	
}
...
void __fastcall TFormaDocRegOstNom::cxGrid1DBTableView1DOCPARTPropertiesButtonClick(
      TObject *Sender, int AButtonIndex)
{
ViborPart();	
}
...
//---------------------------------------------------------------------------
//Внешние модули
void __fastcall TFormaDocRegOstNom::PopupMenuExtModuleClick(TObject *Sender)
{
...
//----------------------------------------------------------------------------
void TFormaDocRegOstNom::RunExternalModule(__int64 id_module, int type_module)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::ActionCloseExecute(TObject *Sender)
{
...
void __fastcall TFormaDocRegOstNom::ActionOKExecute(TObject *Sender)
{
if(Prosmotr==true) return;
if (DM->SaveDoc()==true)
		{
...
void __fastcall TFormaDocRegOstNom::ActionDvRegExecute(TObject *Sender)
{
if(Prosmotr==true) return;
if (DM->SaveDoc()==true)
		{
...
void __fastcall TFormaDocRegOstNom::ActionSaveExecute(TObject *Sender)
{
if(Prosmotr==true) return;
if (DM->SaveDoc()==true)
		{
...
void __fastcall TFormaDocRegOstNom::ActionPrintExecute(TObject *Sender)
{
OpenPrintForm() ;
}
//---------------------------------------------------------------------------
void TFormaDocRegOstNom::OpenFormSpiskaSprProject(void)
{
if (Prosmotr==true) return;
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborProject(void)
{
...
//----------------------------------------------------------------------------
void TFormaDocRegOstNom::OpenFormSpiskaSprBusinessOper(void)
{
if (Prosmotr==true) return;
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::EndViborBusinessOper(void)
{
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegOstNom::NameBusinessOpercxDBButtonEditPropertiesButtonClick(TObject *Sender,
          int AButtonIndex)
{
switch (AButtonIndex)
	{
...
void __fastcall TFormaDocRegOstNom::NameProjectcxDBButtonEditPropertiesButtonClick(TObject *Sender,
          int AButtonIndex)
{
switch (AButtonIndex)
	{

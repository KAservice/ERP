...
//---------------------------------------------------------------------------
int TREM_FormaDocReal::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
if (EditRekvisitBarCode==ERBC_Nom)
	...
	if (ob_bar->OutReady==true)
		...
		DM->DocTPRICE_REM_DREALT->AsFloat=dm_price->GetValuePrice(
						glStrToInt64(DM->DocIDTPRICE_REM_DREAL->AsString),                     //тип цен
						glStrToInt64(DM->DocTIDNOM_REM_DREALT->AsString),  //номенклатура
						glStrToInt64(DM->DocTIDED_REM_DREALT->AsString),    // единица
						DM->DocTKF_REM_DREALT->AsFloat,                   // коэффициент единицы
...
doc->DM->DocPRIM_DREALROZN->AsString="Реализация (ремонт) №"+DM->DocAllNUM_REM_GALLDOC->AsString+
...
doc->DM->DocPRIMREA->AsString="Реализация (ремонт) №"+DM->DocAllNUM_REM_GALLDOC->AsString+
						" от "+ DM->DocAllPOS_REM_GALLDOC->AsString;
...
doc->DM->DocPRIMCHK->AsString="Реализация (ремонт) №"+DM->DocAllNUM_REM_GALLDOC->AsString+
						" от "+ DM->DocAllPOS_REM_GALLDOC->AsString;
doc->DM->Doc->Post();
...
void TREM_FormaDocReal::OpenFormInputBarCode(void)
{
	...
	if (FormaInputText==NULL)
		...
		FormaInputText->TextMessage="Введите штрих-код:";
		FormaInputText->UpdateForm();
		}

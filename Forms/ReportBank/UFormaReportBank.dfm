object FormaReportBank: TFormaReportBank
  Left = 236
  Top = 136
  Caption = #1054#1090#1095#1077#1090' '#1087#1086' '#1073#1072#1085#1082#1091':'
  ClientHeight = 415
  ClientWidth = 553
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -14
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 120
  TextHeight = 16
  object Label1: TLabel
    Left = 30
    Top = 10
    Width = 208
    Height = 32
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1054#1090#1095#1077#1090' '#1087#1086' '#1073#1072#1085#1082#1091':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -28
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label2: TLabel
    Left = 20
    Top = 59
    Width = 14
    Height = 32
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = 'c'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -28
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label3: TLabel
    Left = 276
    Top = 59
    Width = 32
    Height = 32
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1087#1086
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -28
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 59
    Top = 128
    Width = 47
    Height = 16
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1060#1080#1088#1084#1072':'
  end
  object Label6: TLabel
    Left = 39
    Top = 167
    Width = 71
    Height = 16
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1041#1072#1085#1082'. '#1089#1095#1077#1090':'
  end
  object Label10: TLabel
    Left = 319
    Top = 275
    Width = 194
    Height = 16
    Caption = '* '#1090#1086#1083#1100#1082#1086' '#1076#1083#1103' '#1074#1085#1077#1096#1085#1080#1093' '#1086#1090#1095#1077#1090#1086#1074
  end
  object DateNach: TcxDateEdit
    Left = 59
    Top = 70
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Kind = ckDateTime
    TabOrder = 0
    OnKeyPress = DateNachKeyPress
    Width = 209
  end
  object DateCon: TcxDateEdit
    Left = 319
    Top = 70
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Kind = ckDateTime
    TabOrder = 1
    OnKeyPress = DateConKeyPress
    Width = 189
  end
  object OperationComboBox: TcxComboBox
    Left = 256
    Top = 207
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.DropDownListStyle = lsFixedList
    Properties.Items.Strings = (
      #1055#1088#1086#1095#1077#1077
      #1056#1072#1089#1095#1077#1090#1099' '#1089' '#1082#1086#1085#1090#1088#1072#1075#1077#1085#1090#1072#1084#1080
      #1056#1072#1089#1095#1077#1090#1099' '#1089' '#1073#1102#1076#1078#1077#1090#1086#1084)
    TabOrder = 5
    Width = 238
  end
  object OnlyOperationCheckBox: TcxCheckBox
    Left = 59
    Top = 207
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1090#1086#1083#1100#1082#1086' '#1086#1087#1077#1088#1072#1094#1080#1080
    TabOrder = 4
    Width = 149
  end
  object NameBSchetEdit: TcxButtonEdit
    Left = 116
    Top = 164
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameBSchetEditPropertiesButtonClick
    TabOrder = 3
    Text = 'NameBSchetEdit'
    Width = 390
  end
  object NameFirmEdit: TcxButtonEdit
    Left = 116
    Top = 124
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameFirmEditPropertiesButtonClick
    TabOrder = 2
    Text = 'NameFirmEdit'
    Width = 387
  end
  object cxButtonOCB: TcxButton
    Left = 234
    Top = 370
    Width = 92
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1054#1057#1042
    TabOrder = 8
    OnClick = cxButtonOCBClick
  end
  object cxButtonDv: TcxButton
    Left = 334
    Top = 370
    Width = 92
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1044#1074#1080#1078#1077#1085#1080#1103
    TabOrder = 9
    OnClick = cxButtonDvClick
  end
  object cxButtonClose: TcxButton
    Left = 433
    Top = 370
    Width = 93
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Action = ActionClose
    TabOrder = 10
  end
  object cxButton1: TcxButton
    Left = 95
    Top = 370
    Width = 131
    Height = 31
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1042#1085#1077#1096#1085#1080#1081' '#1086#1090#1095#1077#1090
    DropDownMenu = PopupMenuExtModule
    Glyph.Data = {
      
      0E9322280A931E1E01920505001B000000000000000000000000}
    Kind = cxbkDropDown
    TabOrder = 11
  end
  object cxLabel9: TcxLabel
    Left = 5
    Top = 299
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1041#1080#1079#1085#1077#1089' '#1086#1087#1077#1088#1072#1094#1080#1103'*:'
  end
  object cxLabel3: TcxLabel
    Left = 39
    Top = 331
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Caption = #1055#1088#1086#1077#1082#1090'*:'
  end
  object NameBusinessOpercxButtonEdit: TcxButtonEdit
    Left = 139
    Top = 298
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ClickKey = 13
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameBusinessOpercxButtonEditPropertiesButtonClick
    TabOrder = 6
    Text = 'NameBusinessOpercxButtonEdi'
    Width = 375
  end
  object NameProjectcxButtonEdit: TcxButtonEdit
    Left = 139
    Top = 330
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Properties.Buttons = <
      item
        Default = True
        Kind = bkEllipsis
      end
      item
        Caption = 'X'
        Kind = bkText
      end>
    Properties.ClickKey = 13
    Properties.ReadOnly = True
    Properties.OnButtonClick = NameProjectcxButtonEditPropertiesButtonClick
    TabOrder = 7
    Text = 'NameProjectcxButtonEdit'
    Width = 375
  end
  object IBTr: TpFIBTransaction
    TimeoutAction = TARollback
    TRParams.Strings = (
      'read'
      'nowait'
      'read_committed'
      'rec_version')
    TPBMode = tpbDefault
    Left = 408
    Top = 8
  end
  object IBQ: TpFIBDataSet
    SelectSQL.Strings = (
      '         select'
      'NAMEFIRM_PROC, IDFIRM_PROC, NAMEBSCHET_PROC, IDBSCHET_PROC,'
      'SUM(PRIHOD) AS PRIHODSUM,'
      'SUM(RASHOD) AS RASHODSUM,'
      'SUM(PRIHOD_DO_CON) AS PR_DOCON_SUM,'
      'SUM(RASHOD_DO_CON) AS RAS_DOCON_SUM,'
      'SUM(TEC_OST) AS OSTSUM ,'
      
        '(SUM(TEC_OST)-SUM(PRIHOD_DO_CON)+SUM(RASHOD_DO_CON)  -SUM(PRIHOD' +
        ')  +SUM(RASHOD)           )  AS  NACHOSTSUM,'
      
        '(SUM(TEC_OST)-SUM(PRIHOD_DO_CON)+SUM(RASHOD_DO_CON)         )  A' +
        'S  CONOSTSUM'
      'from OCB_RGBANK_FULL (:DATENACH, :DATECON)'
      ''
      'group by '
      ''
      'NAMEFIRM_PROC, IDFIRM_PROC, NAMEBSCHET_PROC, IDBSCHET_PROC'
      '')
    Transaction = IBTr
    Left = 416
    Top = 96
    object IBQPRIHODSUM: TFIBBCDField
      FieldName = 'PRIHODSUM'
      Size = 2
      RoundByScale = True
    end
    object IBQRASHODSUM: TFIBBCDField
      FieldName = 'RASHODSUM'
      Size = 2
      RoundByScale = True
    end
    object IBQPR_DOCON_SUM: TFIBBCDField
      FieldName = 'PR_DOCON_SUM'
      Size = 2
      RoundByScale = True
    end
    object IBQRAS_DOCON_SUM: TFIBBCDField
      FieldName = 'RAS_DOCON_SUM'
      Size = 2
      RoundByScale = True
    end
    object IBQOSTSUM: TFIBBCDField
      FieldName = 'OSTSUM'
      Size = 2
      RoundByScale = True
    end
    object IBQNACHOSTSUM: TFIBBCDField
      FieldName = 'NACHOSTSUM'
      Size = 2
      RoundByScale = True
    end
    object IBQCONOSTSUM: TFIBBCDField
      FieldName = 'CONOSTSUM'
      Size = 2
      RoundByScale = True
    end
    object IBQIDFIRM_PROC: TFIBBCDField
      FieldName = 'IDFIRM_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQIDBSCHET_PROC: TFIBBCDField
      FieldName = 'IDBSCHET_PROC'
      Size = 0
      RoundByScale = True
    end
    object IBQNAMEFIRM_PROC: TFIBWideStringField
      FieldName = 'NAMEFIRM_PROC'
      Size = 100
    end
    object IBQNAMEBSCHET_PROC: TFIBWideStringField
      FieldName = 'NAMEBSCHET_PROC'
      Size = 100
    end
  end
  object Dv: TpFIBDataSet
    SelectSQL.Strings = (
      'select RGBANK_DV .* , TDOC, NUMDOC'
      'from '
      'RGBANK_DV '
      'left outer join GALLDOC on IDDOC_RGBANK_DV= IDDOC'
      'where '
      '( :PARAM_POSNACH <  POSDOC_RGBANK_DV  '
      'and POSDOC_RGBANK_DV < :PARAM_POSCON) '
      'and'
      ' IDFIRM_RGBANK_DV=:PARAM_IDFIRM'
      'and'
      ' IDBSCHET_RGBANK_DV=:PARAM_IDBSCHET'
      ''
      'order by POSDOC_RGBANK_DV')
    Transaction = IBTr
    Left = 416
    Top = 152
    object DvPOSDOC_RGBANK_DV: TFIBDateTimeField
      FieldName = 'POSDOC_RGBANK_DV'
    end
    object DvTDV_RGBANK_DV: TFIBSmallIntField
      FieldName = 'TDV_RGBANK_DV'
    end
    object DvSUM_RGBANK_DV: TFIBBCDField
      FieldName = 'SUM_RGBANK_DV'
      Size = 2
      RoundByScale = True
    end
    object DvOPER_RGBANK_DV: TFIBIntegerField
      FieldName = 'OPER_RGBANK_DV'
    end
    object DvNUMDOC: TFIBIntegerField
      FieldName = 'NUMDOC'
    end
    object DvID_RGBANK_DV: TFIBBCDField
      FieldName = 'ID_RGBANK_DV'
      Size = 0
      RoundByScale = True
    end
    object DvIDDOC_RGBANK_DV: TFIBBCDField
      FieldName = 'IDDOC_RGBANK_DV'
      Size = 0
      RoundByScale = True
    end
    object DvIDFIRM_RGBANK_DV: TFIBBCDField
      FieldName = 'IDFIRM_RGBANK_DV'
      Size = 0
      RoundByScale = True
    end
    object DvIDBSCHET_RGBANK_DV: TFIBBCDField
      FieldName = 'IDBSCHET_RGBANK_DV'
      Size = 0
      RoundByScale = True
    end
    object DvTDOC: TFIBWideStringField
      FieldName = 'TDOC'
      Size = 10
    end
  end
  object ActionList: TActionList
    Images = DM.ImageList1
    Left = 184
    object ActionOpenHelp: TAction
      Caption = 'ActionOpenHelp'
      ShortCut = 112
      OnExecute = ActionOpenHelpExecute
    end
    object ActionClose: TAction
      Caption = #1047#1072#1082#1088#1099#1090#1100
      ImageIndex = 53
      OnExecute = ActionCloseExecute
    end
  end
  object PopupMenuExtModule: TPopupMenu
    Left = 32
    Top = 216
  end
end

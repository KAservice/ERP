...
//---------------------------------------------------------------------------
__fastcall TFormaDocRegPaySale::TFormaDocRegPaySale(TComponent* Owner)
		: TForm(Owner)
{
...
//---------------------------------------------------------------------------
bool TFormaDocRegPaySale::Init(void)
{
bool result=false;
...
InterfaceGlobalCom->kanCreateObject("Oberon.DMDocRegPaySale.1",IID_IDMDocRegPaySale, (void**)&DM);
...
DMTableExtPrintForm->OpenTable(StringToGUID(Global_CLSID_TFormaDocRegPaySaleImpl),false);
...
//---------------------------------------------------------------------------
int TFormaDocRegPaySale::Done(void)
{

return -1;
}
//----------------------------------------------------------------------------
void TFormaDocRegPaySale::UpdateForm(void)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::FormClose(TObject *Sender,
      TCloseAction &Action)
{
...
//---------------------------------------------------------------------------
int TFormaDocRegPaySale::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
...
void TFormaDocRegPaySale::ViborInfBase(void)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::EndViborInfBase(void)
{
...
//---------------------------------------------------------------------------
//выбор фирмы
void TFormaDocRegPaySale::ViborFirm(void)
{
if(Prosmotr==true) return;
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::EndViborFirm(void)
{
	...
	DM->DocT->Edit();
	DM->DocTIDFIRM_DRG_PAYSALET->AsString=FormaSpiskaSprFirm->DM->ElementIDFIRM->AsString;
...
//выбор клиента
//----------------------------------------------------------------------------
void TFormaDocRegPaySale::ViborKlient(void)
{
if(Prosmotr==true) return;
if (FormaSpiskaSprKlient==NULL)
		...
		FormaSpiskaSprKlient->IdKlient=glStrToInt64(DM->DocTIDKLIENT_DRG_PAYSALET->AsString);
		FormaSpiskaSprKlient->UpdateForm();

		}
}
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::EndViborKlient(void)
{

                DM->DocT->Edit();
				DM->DocTIDKLIENT_DRG_PAYSALET->AsString=FormaSpiskaSprKlient->DM->ElementIDKLIENT->AsString;
...
//---------------------------------------------------------------------------
void TFormaDocRegPaySale::OpenPrintForm(void)
{
...
//---------------------------------------------------------------------------
void TFormaDocRegPaySale::OutputZagolovokReport(TSheetEditor *prForm)
{
...
cell->Text="Движения регистра Оплата реализации №"+DM->DocAllNUMDOC->AsString+" от "
...
cell->Text="Основание: "+DM->DocDESCR_DRG_PAYSALE->AsString;
delete cell;
numRow++;
numRow++;
...
//------------------------------------------------------------------------------
void TFormaDocRegPaySale::OutputString(TSheetEditor *prForm)
{
numRow ++;
...
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text=DM->DocTNAME_SDOG->AsString;
//cell->Style->HorzTextAlign = haCENTER;
cell->Style->WordBreak = true;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
cell->Style->Borders->Edges [eRight]->Style=lsThin;
...
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text=DM->DocTDATE_DRG_PAYSALET->AsString;
//cell->Style->HorzTextAlign = haCENTER;
cell->Style->WordBreak = true;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
cell->Style->Borders->Edges [eRight]->Style=lsThin;
...
cell = prForm->SS->ActiveSheet->GetCellObject(numCol, numRow);
cell->Text=DM->DocTSUM_DRG_PAYSALET->AsString;
cell->Style->HorzTextAlign = haRIGHT;
cell->Style->Borders->Edges [eLeft]->Style=lsThin;
cell->Style->Borders->Edges [eRight]->Style=lsThin;
cell->Style->Borders->Edges [eTop]->Style=lsThin;
...
//------------------------------------------------------------------------------
void TFormaDocRegPaySale::OutputPodavalReport(TSheetEditor *prForm)
{
numRow++;
numCol=2;
TcxSSCellObject *cell;
...
void __fastcall TFormaDocRegPaySale::NumberDocKeyDown(TObject *Sender,
...
void __fastcall TFormaDocRegPaySale::FDateDocKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN) FindNextControl((TWinControl *) Sender,true,true,false)->SetFocus();
}
...
void __fastcall TFormaDocRegPaySale::PrimKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{
if(Key==VK_RETURN) FindNextControl((TWinControl *) Sender,true,true,false)->SetFocus();
}
...
void __fastcall TFormaDocRegPaySale::ActionOpenHelpExecute(TObject *Sender)
{
Application->HelpJump("DocRegPaySale");
}
//---------------------------------------------------------------------------



void __fastcall TFormaDocRegPaySale::NameInfBasecxDBButtonEditPropertiesButtonClick(
...
void __fastcall TFormaDocRegPaySale::ActionDeleteStringExecute(TObject *Sender)
{
if(Prosmotr==true) return;
DM->TableDelete();
}
...
void __fastcall TFormaDocRegPaySale::NumDoccxDBTextEditKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN)
	{
...
void __fastcall TFormaDocRegPaySale::PosDoccxDBDateEditKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN)
	{
...
void __fastcall TFormaDocRegPaySale::PrimcxDBTextEditKeyDown(TObject *Sender,
	  WORD &Key, TShiftState Shift)
{
if(Key==VK_RETURN)
	{
...
void TFormaDocRegPaySale::BeforeSaveDoc(void)
{
...
void __fastcall TFormaDocRegPaySale::ActionAddStringExecute(TObject *Sender)
{
...
void __fastcall TFormaDocRegPaySale::cxGrid1DBTableView1NAMEFIRMPropertiesButtonClick(
      TObject *Sender, int AButtonIndex)
{
ViborFirm();	
}
...
void __fastcall TFormaDocRegPaySale::cxGrid1DBTableView1NAMEKLIENTPropertiesButtonClick(
      TObject *Sender, int AButtonIndex)
{
ViborKlient();	
}
...
//---------------------------------------------------------------------------
//Внешние модули
void __fastcall TFormaDocRegPaySale::PopupMenuExtModuleClick(TObject *Sender)
{
...
//----------------------------------------------------------------------------
void TFormaDocRegPaySale::RunExternalModule(__int64 id_module, int type_module)
{
...
void __fastcall TFormaDocRegPaySale::ActionCloseExecute(TObject *Sender)
{
...
void __fastcall TFormaDocRegPaySale::ActionOKExecute(TObject *Sender)
{
if(Prosmotr==true) return;
if (DM->SaveDoc()==true)
		{
...
void __fastcall TFormaDocRegPaySale::ActionDvRegExecute(TObject *Sender)
{
if(Prosmotr==true) return;
if (DM->SaveDoc()==true)
		{
...
void __fastcall TFormaDocRegPaySale::ActionSaveExecute(TObject *Sender)
{
if(Prosmotr==true) return;
if (DM->SaveDoc()==true)
		{
...
void __fastcall TFormaDocRegPaySale::ActionPrintExecute(TObject *Sender)
{
OpenPrintForm() ;
}
//---------------------------------------------------------------------------
void TFormaDocRegPaySale::OpenFormSpiskaSprProject(void)
{
if (Prosmotr==true) return;
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::EndViborProject(void)
{
...
//----------------------------------------------------------------------------
void TFormaDocRegPaySale::OpenFormSpiskaSprBusinessOper(void)
{
if (Prosmotr==true) return;
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::EndViborBusinessOper(void)
{
...
//----------------------------------------------------------------------------
void __fastcall TFormaDocRegPaySale::NameBusinessOpercxDBButtonEditPropertiesButtonClick(TObject *Sender,
          int AButtonIndex)
{
switch (AButtonIndex)
	{
...
void __fastcall TFormaDocRegPaySale::NameProjectcxDBButtonEditPropertiesButtonClick(TObject *Sender,
          int AButtonIndex)
{
switch (AButtonIndex)
	{

...
//----------------------------------------------------------------------------
AnsiString TReportDiscountDocCheck::GetTextQuery(void)
{
...
zapros=zapros+" where dchkt.skchkt!=0  and galldoc.posdoc between :PARAM_POSNACH and :PARAM_POSCON";
        ...
        if (IdFirm!=0)
                {
				zapros=zapros+" and  sfirm.idfirm="+IntToStr(IdFirm);
                }

		if (IdSklad!=0)
                {
				zapros=zapros+" and  ssklad.idsklad="+IntToStr(IdSklad);
                }

		if (IdGrp!=0)
                {
				zapros=zapros+" and  sgrpnom.idgn="+IntToStr(IdGrp);;
                }

        if (IdNom!=0)
                {
				zapros=zapros+" and  dchkt.idnomchkt="+IntToStr(IdNom);
                }

        if (OnlyType==true)
                {
				zapros=zapros+" and  snom.tnom="+IntToStr(TypeNom);
                }
...
//---------------------------------------------------------------------------
AnsiString TReportDiscountDocCheck::GetTextZaprosDoc(void)
{
AnsiString zapros=" select";

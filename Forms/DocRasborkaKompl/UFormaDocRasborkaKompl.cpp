...
//---------------------------------------------------------------------------
int TFormaDocRasborkaKompl::ExternalEvent(IkanUnknown * pUnk,   //интерфейс на дочерний объект
			 ...
			 //	DM->DocTTNOM->AsInteger=FormaSpiskaSprNom->DM->ElementTNOM->AsInteger;
				...
				if (DM->SkladRozn==true)
					...
					DM->DocRPRICE_DRASBKOMPL->AsFloat=
							DMSprPrice->GetValuePrice(glStrToInt64(DM->DocIDRTPRICE_DRASBKOMPL->AsString), //тип цен
											glStrToInt64(DM->DocIDNOM_DRASBKOMPL->AsString),            //номенклатура
											glStrToInt64(FormaSpiskaSprNom->DM->ElementIDBASEDNOM->AsString),             //единица
											DM->DocKF_DRASBKOMPL->AsFloat,                              //коэффициент
	...
	DM->DocKF_DRASBKOMPL->AsFloat=FormaSpiskaSprEd->DM->ElementKFED->AsFloat;
				...
				if (DM->SkladRozn==true)
					...
					DM->DocRPRICE_DRASBKOMPL->AsFloat=
							DMSprPrice->GetValuePrice(glStrToInt64(DM->DocIDRTPRICE_DRASBKOMPL->AsString), //тип цен
											glStrToInt64(DM->DocIDNOM_DRASBKOMPL->AsString),            //номенклатура
											glStrToInt64(DM->DocIDED_DRASBKOMPL->AsString),             //единица
											DM->DocKF_DRASBKOMPL->AsFloat,                              //коэффициент
											0);            //базовая единица
					DMSprPrice->kanRelease();
					}
			 ...
			 //	DM->DocTTNOM->AsInteger=FormaSpiskaSprNom->DM->ElementTNOM->AsInteger;
				...
				if (DM->SkladRozn==true)
					...
					DM->DocTRPRICE_DRASBKOMPLT->AsFloat=
							DMSprPrice->GetValuePrice(glStrToInt64(DM->DocIDRTPRICE_DRASBKOMPL->AsString), //тип цен
											glStrToInt64(DM->DocTIDNOM_DRASBKOMPLT->AsString),            //номенклатура
											glStrToInt64(DM->DocTIDED_DRASBKOMPLT->AsString),             //единица
											DM->DocTKF_DRASBKOMPLT->AsFloat,                              //коэффициент
											glStrToInt64(FormaSpiskaSprNom->DM->ElementIDBASEDNOM->AsString));            //базовая единица
					DMSprPrice->kanRelease();
					}
				else
...
cell->Text="Разборка комплекта №"+DM->DocAllNUMDOC->AsString+" от "
...
cell->Text="Единица: "+DM->DocNAMEED->AsString+
			" количество: "+DM->DocKOL_DRASBKOMPL->AsString+
			" розн. цена: "+DM->DocRPRICE_DRASBKOMPL->AsString;
delete cell;
numRow++;
cell = prForm->SS->ActiveSheet->GetCellObject(2, numRow);
cell->Text="Основание: "+DM->DocDESCR_DRASBKOMPL->AsString;
delete cell;
numRow++;
numRow++;
...
if (glStrToInt64(DM->DocTIDNOM_DRASBKOMPLT->AsString)>0)
	...
	if (FormaElementeSprPart==0)
		...
		FormaElementeSprPart->DM->ElementDOCPART->AsString="Разборка комплекта №"+DM->DocAllNUMDOC->AsString+" от " +DM->DocAllPOSDOC->AsString;

...
Query->Close();
Query->SQL->Clear();
Query->SQL->Add(" select VALUE_TIMESTAMP_SETUP from SETUP where ID_SETUP=313");
Query->ExecQuery();
...
Query->SQL->Clear();
Query->SQL->Add(" select VALUE_TIMESTAMP_SETUP from SETUP where ID_SETUP=315");
Query->ExecQuery();
if (Query->FieldByName("VALUE_TIMESTAMP_SETUP")->AsString=="" )
        {
        }
...
AnsiString zapros="select rem_galldoc.id_rem_galldoc, rem_galldoc.pos_rem_galldoc from rem_galldoc where";
zapros=zapros+" (rem_galldoc.pr_rem_galldoc=0 or rem_galldoc.pr_rem_galldoc IS NULL)";

if (PosZapretaRedakt > PosNull)
        {
		zapros=zapros+" and rem_galldoc.pos_rem_galldoc>'"+DateTimeToStr(PosZapretaRedakt)+"'";
        }
zapros=zapros+" order by rem_galldoc.pos_rem_galldoc";
Query->Close();
Query->SQL->Clear();
...
if (!Query->Eof)
        {
		PosFirstNoDvRegDoc=Query->FieldByName("pos_rem_galldoc")->AsDateTime;
        }
Query->Close();
...
if (regime=="OnlyDvReg")
	...
	AnsiString zapros="select count(rem_galldoc.id_rem_galldoc) as COUNTREC from rem_galldoc ";
	zapros=zapros+" where (rem_galldoc.pr_rem_galldoc=0 or rem_galldoc.pr_rem_galldoc IS NULL)  and rem_galldoc.pos_rem_galldoc>=:PARAM_POSDOC";
	Query->SQL->Add(zapros);
...
bool result=false;
		...
		QueryUpdate->SQL->Add(" update setup set VALUE_SETUP=1 where ID_SETUP=314");
		try
			{
...
bool result=false;
		...
		QueryUpdate->Close();
		QueryUpdate->SQL->Clear();
		QueryUpdate->SQL->Add(" update setup set VALUE_SETUP=null where ID_SETUP=314");
		try
			{
			QueryUpdate->ExecQuery();
			TransactionUpdate->Commit();
...
bool result=false;
		...
		QueryUpdate->Close();
		QueryUpdate->SQL->Clear();
		QueryUpdate->SQL->Add(" update setup set VALUE_TIMESTAMP_SETUP=null where ID_SETUP=315");
		try
			{
			QueryUpdate->ExecQuery();
			TransactionUpdate->Commit();
...
while (!GurAllDoc->Eof)
		{
				try
                        {
						if (RunComandDvRegDoc(Trim(GurAllDocTDOC_REM_GALLDOC->AsString),
								GurAllDocID_REM_GALLDOC->AsInt64)!=true)
								{
								message="Не удалось провести документ "+
								glGetNameDocRemont(GurAllDocTDOC_REM_GALLDOC->AsString)+
								"  №"+GurAllDocNUM_REM_GALLDOC->AsString+
								" от "+GurAllDocPOS_REM_GALLDOC->AsString;
						...
						glGetNameDocRemont(GurAllDocTDOC_REM_GALLDOC->AsString)+"  №"+GurAllDocNUM_REM_GALLDOC->AsString+
						" от "+GurAllDocPOS_REM_GALLDOC->AsString+
                        "  "+exception.Message;
                        glSaveProtocol(message);
						Memo1->Lines->Add(message);
...
AnsiString zapros="select rem_galldoc.id_rem_galldoc, rem_galldoc.pos_rem_galldoc from rem_galldoc where";
zapros=zapros+" (rem_galldoc.pr_rem_galldoc=0 or rem_galldoc.pr_rem_galldoc IS NULL)";
zapros=zapros+" and rem_galldoc.pos_rem_galldoc>'"+DateTimeToStr(Now()-1)+"'";
...
if (!Query->Eof)
		{
		PosStart=Query->FieldByName("pos_rem_galldoc")->AsDateTime;
		}
...
if (PosStart > PosNull && PosStart >= PosZapretaRedakt )
		...
		while (!GurAllDoc->Eof)
                {
				RunComandDvRegDoc(Trim(GurAllDocTDOC_REM_GALLDOC->AsString),
								GurAllDocID_REM_GALLDOC->AsInt64);
...
while (!GurAllDoc->Eof)
		...
		try
                {
				if (RunComandCancelDvRegDoc(Trim(GurAllDocTDOC_REM_GALLDOC->AsString),
								GurAllDocID_REM_GALLDOC->AsInt64)!=true)
						{
...
if (Query->FieldByName("VALUE_TIMESTAMP_SETUP")->AsString=="")
		...
		UnicodeString zapros="select count(rem_galldoc.id_rem_galldoc) AS KOLPRDOC   from rem_galldoc";
		zapros=zapros+" where  (rem_galldoc.pr_rem_galldoc>0)  AND  (rem_galldoc.pos_rem_galldoc-CAST('"+DateTimeToStr(gr_posled)+"' AS TIMESTAMP)>0.0001)";
		Query->SQL->Clear();
		Query->SQL->Add(zapros);
		Query->ExecQuery();
		...
		Query->Close();
				...
				if (KolProvDoc==0)
						...
						result=false;
                                ...
                                Query->Close();
								Query->SQL->Clear();
								Query->SQL->Add("select rem_galldoc.tdoc_rem_galldoc, rem_galldoc.num_rem_galldoc, rem_galldoc.pos_rem_galldoc from REM_GALLDOC where rem_galldoc.pr_rem_galldoc>0  AND rem_galldoc.pos_rem_galldoc>:PARAM_POSDOC");
								Query->ParamByName("PARAM_POSDOC")->AsDateTime=PosStart;
								Query->ExecQuery();
								int kol=0;
								...
								while(!Query->Eof && kol<5)
										{
										message=message+glGetNameDocRemont(Trim(Query->FieldByName("tdoc_rem_galldoc")->AsString))
										+" №"+Query->FieldByName("num_rem_galldoc")->AsString
										+" от "+Query->FieldByName("pos_rem_galldoc")->AsString+"\n";
										Query->Next(); kol++;
										}
								Query->Close();
								glSaveProtocol(message);
...
if (PosStart < Now()-100)
	...
	Query->Close();
	Query->SQL->Clear();
	Query->SQL->Add(" select VALUE_SETUP from SETUP where ID_SETUP=343");
	Query->ExecQuery();
	if (Query->FieldByName("VALUE_SETUP")->AsString=="1")
		...
		{
		glSaveProtocol("Флаг подтверждения не установлен Константа 343");
		Memo1->Lines->Add("Флаг подтверждения не установлен Константа 343");
...
{
bool result=false;
UnicodeString name_procedure=glGetNameProcedureCancelDvRegRemontSystem(type_doc);
...
{
bool result=false;
UnicodeString name_procedure=glGetNameProcedureDvRegRemontSystem(type_doc);

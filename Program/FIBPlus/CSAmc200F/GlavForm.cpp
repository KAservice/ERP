...
while(!DMSprOborud->Table->Eof)
        ...
        if (Trim(DMSprOborud->TableTYPE_OBORUD->AsString)=="kkm")
				...
				if (DMSprOborud->GetParameterPoTable("SetNumberKKM")=="")
					{
					ShowMessage("Не задан сетевой номер в справочнике Оборудование. Параметр SetNumberKKM. По умолчанию последние 2 цифры заводского номера!");
					}
				else
					{
...
switch (amc->EventKKM)
        ...
        case amc->ZaprosPoCoduEvent:      //запрос по коду
                ...
                StrokaMemo=StrokaMemo+" Номер КЛ: "+IntToStr(amc->NumberKl);
        ...
        case amc->UnknownEvent:
                {
                FormGl->LabelSoobshenie->Font->Color=clRed;
                FormGl->LabelSoobshenie->Caption="Неизвестное событие от ККМ!";
                FormGl->RE->Lines->Add("Неизвестное событие от ККМ!");

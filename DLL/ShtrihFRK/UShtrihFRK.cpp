...
extern "C" bool __declspec (dllexport) PrintString(WideString string,
                                                int size_font,       //1234
...
Variant  FR;
int GetNumberBaudRate(UnicodeString br);
int EndPrint(void);
...
UnicodeString res=FR.OlePropertyGet("SerialNumber");
result=res;
return result;
...
WideString result="";
FR.OleProcedure("GetLongSerialNumberAndLongRNM");
UnicodeString num=FR.OlePropertyGet("RNM");
result=num;
...
//---------------------------------------------------------------------------
bool  PrintString(WideString string,
												int size_font,       //1234
												int style,           //1-жирный 0 обычный   2 курсив 3 жирный курсив
												int alignment,       //0 лево 1 центр 2 право
												bool ch_lenta,       //на чековой ленте
...
// ***************************************************************************
		...
		switch ( size_font)
				...
				case  1:
						{
						if(s.Length()>36) s=s.SetLength(36);
                        switch ( alignment)
                                {
								case  0:      //лево
                                        {
                ...
                case  3:
						{
						if(s.Length()>18) s=s.SetLength(18);
                        switch ( alignment)
                                {
                                case  0:      //лево
                                        {
...
WideString result="";
FR.OleProcedure("GetECRStatus");
UnicodeString time=FR.OlePropertyGet("Time");
result=time;
...
WideString result="";
FR.OleProcedure("GetECRStatus");
UnicodeString date=FR.OlePropertyGet("Date");
result=date;

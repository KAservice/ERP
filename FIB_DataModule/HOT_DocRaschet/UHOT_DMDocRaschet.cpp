...
//---------------------------------------------------------------------------
__fastcall THOT_DMDocRaschet::THOT_DMDocRaschet(TComponent* Owner)
        : TDataModule(Owner)
{
...
//---------------------------------------------------------------------------
bool THOT_DMDocRaschet::Init(void)
{
bool result=false;
...
//----------------------------------------------------------------------------
int THOT_DMDocRaschet::Done(void)
{

return -1;
}
//---------------------------------------------------------------------------
void __fastcall THOT_DMDocRaschet::DataModuleDestroy(TObject *Sender)
{
DocAll->Close();
Doc->Close();
DocT->Close();
...
//----------------------------------------------------------------------------
void THOT_DMDocRaschet::NewDoc(void)
{
DocAll->Open();
Doc->Open();
DocAll->Append();
...
DocIDDOCH_HOT_DRASCHET->AsString=IdDoc;
Doc->Post();

DocAllPOS_HOT_GALLDOC->AsDateTime=Now();
DocAllTDOC_HOT_GALLDOC->AsString="RASCHET";
...
NumDoc->ParamByName("PARAM_POS")->AsDateTime=dm_num->GetPosNachNumDoc("RASCHET");
NumDoc->Open();
...
//----------------------------------------------------------------------------
void THOT_DMDocRaschet::OpenDoc(__int64 Id)
{
...
//----------------------------------------------------------------------------
void THOT_DMDocRaschet::CloseDoc(void)
{
...
//----------------------------------------------------------------------------
bool THOT_DMDocRaschet::SaveDoc(void)
{
//DocT->First();
...
bool THOT_DMDocRaschet::DeleteDoc(__int64 id)
{
bool res=false;
...
//-----------------------------------------------------------------------------
void THOT_DMDocRaschet::AddString(void)
{
DocT->Append();
DocTIDDOCH_HOT_DRASCHETT->AsString=DocAllID_HOT_GALLDOC->AsString;
DocT->Post();

}
//-----------------------------------------------------------------------------
void THOT_DMDocRaschet::DeleteString(void)
{
DocT->Delete();
}
//-----------------------------------------------------------------------------
double THOT_DMDocRaschet::Summa(void)
{
...
while (!DocT->Eof)
        {
		SummaDoc=SummaDoc+DocTSUM_HOT_DRASCHETT->AsFloat;
        DocT->Next();
        }
...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
bool THOT_DMDocRaschet::DvRegDoc(void)
{
bool result=false;
if (Prosmotr==true)
		{
...
//-----------------------------------------------------------------------------
bool THOT_DMDocRaschet::CancelDvRegDoc(void)
{
bool result=false;
...
void __fastcall THOT_DMDocRaschet::DocNewRecord(TDataSet *DataSet)
{
DocIDDOCH_HOT_DRASCHET->AsString=DocAllID_HOT_GALLDOC->AsString;

if (glStrToInt64(DocAllIDBASE_HOT_GALLDOC->AsString)!=0)
	{
	DocIDBASE_HOT_DRASCHET->AsString=DocAllIDBASE_HOT_GALLDOC->AsString;
	}
}
//---------------------------------------------------------------------------

void __fastcall THOT_DMDocRaschet::DocTNewRecord(TDataSet *DataSet)
{
DocTIDDOCH_HOT_DRASCHETT->AsString=DocAllID_HOT_GALLDOC->AsString;

	if (glStrToInt64(DocAllIDBASE_HOT_GALLDOC->AsString)!=0)
		{
		DocTIDBASE_HOT_DRASCHETT->AsString=DocAllIDBASE_HOT_GALLDOC->AsString;
		}
}

//---------------------------------------------------------------------------

void __fastcall THOT_DMDocRaschet::DocTCalcFields(TDataSet *DataSet)
{
DocTRECNO->AsInteger=DocT->RecNo;
}
//---------------------------------------------------------------------------

void __fastcall THOT_DMDocRaschet::DocTBeforeDelete(TDataSet *DataSet)
{
OldSummaStr=DocTSUM_HOT_DRASCHETT->AsFloat;
}
//---------------------------------------------------------------------------

void __fastcall THOT_DMDocRaschet::DocTAfterDelete(TDataSet *DataSet)
{
NewSummaStr=0;
...
void __fastcall THOT_DMDocRaschet::DocAllPOS_HOT_GALLDOCChange(TField *Sender)
{
IDMSetup * DMSetup;
InterfaceGlobalCom->kanCreateObject("Oberon.DMSetup.1",IID_IDMSetup,
													(void**)&DMSetup);
...
void __fastcall THOT_DMDocRaschet::DocTKOL_HOT_DRASCHETTChange(TField *Sender)
{
OldSummaStr=DocTSUM_HOT_DRASCHETT->AsFloat;

DocTSUM_HOT_DRASCHETT->AsFloat=NewSummaStr=DocTKOL_HOT_DRASCHETT->AsFloat*DocTPRICE_HOT_DRASCHETT->AsFloat;
...
void __fastcall THOT_DMDocRaschet::DocTPRICE_HOT_DRASCHETTChange(TField *Sender)
{
OldSummaStr=DocTSUM_HOT_DRASCHETT->AsFloat;
...
//---------------------------------------------------------------------------
void THOT_DMDocRaschet::AddStringReal(TDateTime pos_nach, TDateTime pos_con,
...
//
//	double tek_kol=0;
//	int id_tek_type_price=0;
//
//
//	int kol_dob_strok=0;
//	int id_new_type_price=0;
//
...
//-----------------------------------------------------------------------------
void THOT_DMDocRaschet::AddStringNewReal( TDateTime pos_nach, TDateTime pos_con,
							__int64 id_nomer, AnsiString name_nomer,
							__int64 id_cat,  AnsiString name_cat,
							__int64 id_type_pos, AnsiString name_type_pos,
							__int64 id_guest, AnsiString name_guest,
...
//
//DocT->Append();
//DocTTUSL_HOT_DRASCHETT->AsInteger=type_usl;
//if(id_nom!=0)
//	{
//	DocTIDNOM_HOT_DRASCHETT->AsInt64=id_nom;
//	}
//DocTNAMENOM->AsString=name_nom;
//if (id_type_price!=0)
//	{
//	DocTIDTPRICE_HOT_DRASCHETT->AsInt64=id_type_price;
//	}
//if(id_ed!=0)
//	{
//	DocTIDED_HOT_DRASCHETT->AsInt64=id_ed;
//	}
//
//DocTKF_HOT_DRASCHETT->AsFloat=kf;
//DocTNAMEED->AsString=name_ed;
//DMSprPrice->FindElement(DocTIDTPRICE_HOT_DRASCHETT->AsInt64,
//								DocTIDNOM_HOT_DRASCHETT->AsInt64);
//
//DocTPRICE_HOT_DRASCHETT->AsFloat=DMSprPrice->ElementZNACH_PRICE->AsFloat
//									*DocTKF_HOT_DRASCHETT->AsFloat;
//
//DocTKOL_HOT_DRASCHETT->AsFloat=kol;
//DocTPOSNACH_HOT_DRASCHETT->AsDateTime=pos_nach;
//DocTPOSCON_HOT_DRASCHETT->AsDateTime=pos_con;
//if(id_nomer!=0)
//	{
//	DocTIDNOMER_HOT_DRASCHETT->AsInt64=id_nomer;
//	}
//if (id_cat)
//	{
//	DocTIDCAT_HOT_DRASCHETT->AsInt64=id_cat;
//	}
//if(id_type_pos)
//	{
//	DocTIDTPOS_HOT_DRASCHETT->AsInt64=id_type_pos;
//	}
//if(id_guest!=0)
//	{
//	DocTIDGUEST_HOT_DRASCHETT->AsInt64=id_guest;
//	}
//
...
//--------------------------------------------------------------------------
double THOT_DMDocRaschet::GetKolOsnUslugGuest(__int64 id_guest)
{
double result=0;
...
while (!SpisokGuestOsnUslug->Eof)
	{
	if (SpisokGuestOsnUslugIDKLIENT->AsInt64==id_guest)
		{
...
//---------------------------------------------------------------------------
double THOT_DMDocRaschet::GetSumDopUslugGuest(__int64 id_guest)
{
double result=0;
...
while (!SpisokGuestDopUslug->Eof)
	{
	if (SpisokGuestDopUslugIDKLIENT->AsInt64==id_guest)
		{
...
//----------------------------------------------------------------------------
void __fastcall THOT_DMDocRaschet::DocAllIDBASE_HOT_GALLDOCChange(
      TField *Sender)
{
if (glStrToInt64(DocAllIDBASE_HOT_GALLDOC->AsString)!=0)
	{
...
void __fastcall THOT_DMDocRaschet::DocTTUSL_HOT_DRASCHETTGetText(TField *Sender, UnicodeString &Text,
          bool DisplayText)
{
if (DocTTUSL_HOT_DRASCHETT->AsInteger==0)
		{
		Text="Не зад.";
		}
if (DocTTUSL_HOT_DRASCHETT->AsInteger==1)
		{
		Text="Осн.";
		}
if (DocTTUSL_HOT_DRASCHETT->AsInteger==2)
		{
		Text="Брон.";
		}
if (DocTTUSL_HOT_DRASCHETT->AsInteger==3)
		{
		Text="Доп.";
		}
if (DocTTUSL_HOT_DRASCHETT->AsInteger==4)
		{
		Text="Пит.";
		}
}
...
void __fastcall THOT_DMDocRaschet::DocTTUSL_HOT_DRASCHETTSetText(TField *Sender, const UnicodeString Text)

{
if(Text=="Не зад.")
		{
		DocTTUSL_HOT_DRASCHETT->AsInteger=0;
		}
if(Text=="Осн.")
		{
		DocTTUSL_HOT_DRASCHETT->AsInteger=1;
		}
if(Text=="Брон.")
		{
		DocTTUSL_HOT_DRASCHETT->AsInteger=2;
		}
if(Text=="Доп.")
		{
		DocTTUSL_HOT_DRASCHETT->AsInteger=3;
		}
if(Text=="Пит.")
		{
		DocTTUSL_HOT_DRASCHETT->AsInteger=4;
		}
}
//---------------------------------------------------------------------------

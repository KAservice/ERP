...
if (id_grp!=0)
        {
		ElementIDGRP_HLP_IMAGE_GRP->AsString=id_grp;
		Element->Post();
        }
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall THLP_DMSprImageGrp::TableCalcFields(TDataSet *DataSet)
{
...
pFIBQ->Close();
pFIBQ->SQL->Clear();
pFIBQ->SQL->Add("select hlp_image_grp.id_hlp_image_grp from hlp_image_grp where hlp_image_grp.name_hlp_image_grp=:PARAM_NAME");
pFIBQ->ParamByName("PARAM_NAME")->AsString=Trim(name);
pFIBQ->ExecQuery();
result=glStrToInt64(pFIBQ->FieldByName("id_hlp_image_grp")->AsString);
pFIBQ->Close();

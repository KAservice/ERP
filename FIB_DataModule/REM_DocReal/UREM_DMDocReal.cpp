...
//---------------------------------------------------------------------------
__fastcall TREM_DMDocReal::TREM_DMDocReal(TComponent* Owner)
        : TDataModule(Owner)
{
...
//-----------------------------------------------------------------
void __fastcall TREM_DMDocReal::DataModuleCreate(TObject *Sender)
{
SummaDoc=0;
OldSummaStr=0;
NewSummaStr=0;
...
//----------------------------------------------------------------------------
int TREM_DMDocReal::Done(void)
{

return -1;
}
//---------------------------------------------------------------------------
void __fastcall TREM_DMDocReal::DataModuleDestroy(TObject *Sender)
{
DocAll->Close();
Doc->Close();
DocT->Close();
...
//----------------------------------------------------------------------------
void TREM_DMDocReal::NewDoc(void)
{
DocAll->Active=false;
Doc->Active=false;
DocT->Active=false;
...
DocIDDOC_REM_DREAL->AsString=IdDoc;
DocAllPOS_REM_GALLDOC->AsDateTime=Now();
DocAllTDOC_REM_GALLDOC->AsString="REMREAL";
...
if(glStrToInt64(DM_Connection->UserInfoIDTPRICE_USER->AsString)!=0)
	{
	DocIDTPRICE_REM_DREAL->AsString=DM_Connection->UserInfoIDTPRICE_USER->AsString;
	}

if (IdTypePrice!=0)
	{
	DocIDTPRICE_REM_DREAL->AsString=IdTypePrice;
	}
Doc->Post();
...
//----------------------------------------------------------------------------
int TREM_DMDocReal::GetNumberNewDoc(void)
{
int result=0;
//получим номер документа
NumDoc->Active=false;
...
NumDoc->ParamByName("PARAM_POS")->AsDateTime=dm_num->GetPosNachNumDoc("REMREAL");
NumDoc->Open();
...
//----------------------------------------------------------------------------
bool TREM_DMDocReal::OpenDoc(__int64 Id)
{
bool result=false;
DocAll->Active=false;
Doc->Active=false;
...
void TREM_DMDocReal::ValidateProsmotr(void)
{
...
//----------------------------------------------------------------------------
void TREM_DMDocReal::CloseDoc(void)
{
...
//----------------------------------------------------------------------------
bool TREM_DMDocReal::SaveDoc(void)
{
bool Res=false;
IDMSetup * DMSetup;
InterfaceGlobalCom->kanCreateObject("Oberon.DMSetup.1",IID_IDMSetup,
...
//-----------------------------------------------------------------------------
bool TREM_DMDocReal::BeforeSaveDoc(void)
{
bool result=true;
...
//-----------------------------------------------------------------------------
bool TREM_DMDocReal::AfterSaveDoc(void)
{
bool result=true;
...
//-----------------------------------------------------------------------------
void TREM_DMDocReal::AddDocNewString(void)
{
DocT->Append();

}
//-----------------------------------------------------------------------------
void TREM_DMDocReal::DeleteStringDoc(void)
{
DocT->Delete();
}
//-----------------------------------------------------------------------------
double TREM_DMDocReal::Summa(void)
{
...
while (!DocT->Eof)
		{
		SummaDoc=SummaDoc+DocTSUM_REM_DREALT->AsFloat;
		DocT->Next();
		}
...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
bool TREM_DMDocReal::DvRegDoc(void)
{
bool result=false;
if (Prosmotr==true)
		{
...
//-----------------------------------------------------------------------------
bool TREM_DMDocReal::CancelDvRegDoc(void)
{
bool result=false;
...
bool TREM_DMDocReal::DeleteDoc(__int64 id)
{
bool res=false;
IdDoc=id;
OpenDoc(id);
...
void __fastcall TREM_DMDocReal::DocNewRecord(TDataSet *DataSet)
{
DocIDDOC_REM_DREAL->AsString=DocAllID_REM_GALLDOC->AsString;
if (glStrToInt64(DocAllID_REM_GALLDOC->AsString)!=0)
	{
	DocIDBASE_REM_DREAL->AsString=DocAllIDBASE_REM_GALLDOC->AsString;
	}
}
//---------------------------------------------------------------------------

void __fastcall TREM_DMDocReal::DocTCalcFields(TDataSet *DataSet)
{
DocTRECNO->AsInteger=DocT->RecNo;
}
//---------------------------------------------------------------------------

void __fastcall TREM_DMDocReal::DocTAfterDelete(TDataSet *DataSet)
{
NewSummaStr=0;
...
void __fastcall TREM_DMDocReal::DocTBeforeDelete(TDataSet *DataSet)
{
OldSummaStr=DocTSUM_REM_DREALT->AsFloat;
}
//---------------------------------------------------------------------------

void __fastcall TREM_DMDocReal::DocTNewRecord(TDataSet *DataSet)
{
DocTIDDOC_REM_DREALT->AsString=DocAllID_REM_GALLDOC->AsString;
if (glStrToInt64(DocAllIDBASE_REM_GALLDOC->AsString)!=0)
	{
	DocTIDBASE_REM_DREALT->AsString=DocAllIDBASE_REM_GALLDOC->AsString;
	}
}
//---------------------------------------------------------------------------

void TREM_DMDocReal::OpenDocOsn(void)
{
DocOsn->Active=false;
...
void __fastcall TREM_DMDocReal::DocAllPOS_REM_GALLDOCChange(TField *Sender)
{
IDMSetup * DMSetup;
InterfaceGlobalCom->kanCreateObject("Oberon.DMSetup.1",IID_IDMSetup,
													(void**)&DMSetup);
...
void __fastcall TREM_DMDocReal::DocTPRICE_REM_DREALTChange(TField *Sender)
{


OldSummaStr=DocTSUM_REM_DREALT->AsFloat;

DocTSUM_REM_DREALT->AsFloat=NewSummaStr=DocTKOL_REM_DREALT->AsFloat*DocTPRICE_REM_DREALT->AsFloat;
...
void __fastcall TREM_DMDocReal::DocTKOL_REM_DREALTChange(TField *Sender)
{
OldSummaStr=DocTSUM_REM_DREALT->AsFloat;
...
void __fastcall TREM_DMDocReal::DocTPRSK_REM_DREALTChange(TField *Sender)
{
OldSummaStr=DocTSUM_REM_DREALT->AsFloat;
...
void __fastcall TREM_DMDocReal::DocAllIDBASE_REM_GALLDOCChange(TField *Sender)
{
if (Doc->Active==true)
	{
	if (glStrToInt64(DocAllIDBASE_REM_GALLDOC->AsString)!=0)
...
void __fastcall TREM_DMDocReal::DocOsnTDOC_REM_GALLDOCGetText(TField *Sender, UnicodeString &Text,
          bool DisplayText)
{

...
{
int result=0;
if (id_class==CLSID_TDMStSpr1Impl)
	{
	TDMStSpr1CF * ob=new TDMStSpr1CF();
...
if (icom->kanAddClass(CLSID_TDMStSpr1Impl,ProgId_DMStSpr1,"StDMSpr.dll")!=-1)
	{
	ShowMessage("Error! Falid register class -"+UnicodeString(ProgId_DMStSpr1)+" !");
	}

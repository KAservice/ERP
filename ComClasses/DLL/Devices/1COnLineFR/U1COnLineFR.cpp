...
//-----------------------------------------------------------------------------
bool T1COnLineFR::InitDocument(int type_doc)
{
bool result=true;
...
//-----------------------------------------------------------------------
UnicodeString T1COnLineFR::GetMetodsList(void)
{
UnicodeString result="";

long kol_metod=ObjectVC->GetNMethods();
...
for (long i=0; i<kol_metod;i++)
	...
	int number_metod=i;
	wchar_t * name_metod;
	*name_metod=*ObjectVC->GetMethodName(i,1);
	l->Add(IntToStr(number_metod)+" "+UnicodeString());
	delete name_metod;
	}
...
for (long i=0; i<kol_metod;i++)
	...
	int number_metod=i;
	wchar_t * name_metod;
	*name_metod=*ObjectVC->GetMethodName(i,0);
	l->Add(IntToStr(number_metod)+" "+UnicodeString());
	delete name_metod;
	}
...
//------------------------------------------------------------------------
bool T1COnLineFR::GetParameter(UnicodeString name_parameter, TkasVariant * value_parameter)
{
bool result=true;
...
//--------------------------------------------------------------------------
bool T1COnLineFR::SetParameter(UnicodeString name_parameter, TkasVariant * value_parameter)
{
bool result=true;
